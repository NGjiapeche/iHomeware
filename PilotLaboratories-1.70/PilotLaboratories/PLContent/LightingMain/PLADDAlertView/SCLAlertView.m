...
-(void)viewWillLayoutSubviews
{
    ...
    self.view.frame = r;
    _contentView.frame = CGRectMake(0.0f, kCircleHeight / 4 - 44, kWindowWidth, kWindowHeight);
    _circleViewBackground.frame = CGRectMake(kWindowWidth / 2 - kCircleHeightBackground / 2, kCircleBackgroundTopPosition - 44, kCircleHeightBackground, kCircleHeightBackground);
    _circleViewBackground.layer.cornerRadius = _circleViewBackground.frame.size.height / 2;
    _circleView.frame = CGRectMake(kWindowWidth / 2 - kCircleHeight / 2, kCircleTopPosition - 44, kCircleHeight, kCircleHeight);
...
- (void)buttonTapped:(SCLButton *)btn
{
    ...
    {
        DebugLog(@"Unknown action type for button");
    }
    [self hideView];
}
...
-(SCLAlertViewResponder *)showTitle:(UIViewController *)vc title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(SCLAlertViewStyle)style
{
    ...
    if(_soundURL != nil)
    {
        if (_audioPlayer == nil)
        {
            DebugLog(@"You need to set your sound file first");
        }
        else
        {
            [_audioPlayer play];
